...
> 历史示例说明：下列“分享奖励 10 次，三个月有效”仅用于解释多来源额度的 FIFO 消费模型，不是现行产品规则。现行邀请奖励已由 2026-09-23 后续决策修订为 30 次、自发放之日起一年有效。
...
## 2026-09-23 后续决策记录：邀请新人资格与邀请业务事实

本节不改写 2026-07-21 原始 ADR 或 2026-08-28 当时的后续记录，而是以更新日期明确取代其中“必须最终创建新的 `users` 记录”这一旧定义。现行产品规则以 `docs/CONTENT_ACCESS_POLICY.md` 的 F 节为准，功能正式名称为“邀请新用户注册成功得30次”。

### 新人资格定义

- 新人资格以手机号是否首次完成手机号快捷注册为准，不以本次是否新建 `users` 行为准。
- 普通微信登录和手机号快捷注册是两个不同业务事实。只有微信绑定、从未完成手机号注册的空壳 `users` 不会自动丧失邀请新人资格。
- 手机号已经绑定过任何现有账号时不算新人；新微信身份合并到已有手机号账号时不奖励邀请人。
- 自邀不奖励邀请人，但如果手机号确实首次注册，新用户仍获得普通 `REGISTER_BONUS +30`。

### 两张表边界

第一版推荐拆分为两类业务事实：

1. 邀请分享凭证表：记录邀请人、token 安全标识、创建时间、7 天到期时间、撤销状态及审计时间；同一有效凭证可被多个新人使用。
2. 邀请注册关系表：记录注册前候选、最终邀请人和新人、手机号首次注册资格结果、永久锁定时间、5 次上限占位、奖励状态、奖励额度、权益流水关联、失败恢复和人工复核状态。

`entitlement_transactions` 继续记录 `REGISTER_BONUS` 与 `SHARE_REWARD` 的权益结果，但不能替代上述邀请业务事实。邀请关系必须具备单新人唯一约束，邀请奖励流水必须通过稳定 invitation id 建立严格幂等键和 `source_id` 关联。

### 凭证与历史保留

- 邀请 token 和注册前 `candidateReceipt` 均为 bearer credential，必须用密码学安全随机数高熵生成。
- 数据库只保存带独立密钥的摘要，不保存明文 bearer credential；邀请密钥不得与 JWT、手机号、支付、兑换码或其他业务密钥复用。
- 注册前只允许最后一次有效邀请覆盖候选；过期、撤销或无效邀请不得覆盖仍有效候选。
- 手机号注册成功后邀请关系永久锁定，不允许补录、更换或覆盖。token 到期不得导致已锁定关系、奖励流水或审计记录被删除。

### 奖励与上限

- 新人继续获得自发放之日起一年有效的 `REGISTER_BONUS +30`。
- 邀请人获得在原余额上增加的、同样自发放之日起一年有效的 `SHARE_REWARD +30`，不是把余额重置为 30 次。
- 每位邀请人最多成功获得 5 次、累计 150 次。第 6 位及之后的新用户仍可完成注册并取得自己的新人权益，只是不再产生邀请人奖励。
- 推荐采用两阶段幂等状态机：第一事务锁定手机号首次注册事实、普通新人权益、邀请关系和上限名额；第二事务原子写入 `SHARE_REWARD` 权益流水并完成奖励状态。持久化待发状态由补偿任务重试。

第一批开发前必须核对身份事务与权益事务使用的连接池及事务连接复用方式，确认能够在需要时安全注入同一连接。微信外部 HTTP 不得在持有数据库事务或行锁期间执行。
