...
| 微信虚拟支付订单基础链路 | ✅ 已实现，正式开放仍受发布与验收边界约束 |
| “获取学习权益”页会员购买及本机购买记录 | ✅ 已实现 |
| 自动续费 | ⬜ 第一版不开发 |
| 活动奖励 | ⬜ 未完成 |
...
第一版购买规则已经锁定；虚拟支付订单基础链路、“获取学习权益”页会员购买和本机购买记录已经实现，但正式开放仍需满足对应发布、配置、审核和验收边界：

- 商品暂定 30 元，单次购买固定 `30 × 24` 小时，不做自动连续包月。
- 允许多次购买，按照现有会员 grant 排期顺序叠加。
- 购买入口已经位于统一“获取学习权益”页，并提供购买确认交互。
...
第一版“邀请新用户注册成功得30次”规则已经锁定但尚未实现。新人资格以手机号是否首次完成注册为准，不要求本次新建 `users`：只有微信绑定、从未完成手机号注册的空壳账号仍可认定为邀请新人。手机号已绑定任何现有账号或新微信身份合并到已有手机号账号时，不奖励邀请人。有效新人继续获得一年有效的 `REGISTER_BONUS +30`，邀请人获得在原余额上增加的一年有效 `SHARE_REWARD +30`，累计最多 5 次；第 6 位及之后的新用户仍正常获得自己的新人权益。
...
完整、现行产品规则以 `docs/CONTENT_ACCESS_POLICY.md` 的 F 节为权威来源，功能正式名称统一为“邀请新用户注册成功得30次”，不得表述为“分享即得30次”。

- 普通微信登录和手机号快捷注册是不同业务事实。是否存在 `users` 行不再单独决定新人资格；权威事实是该手机号此前是否完成过手机号注册。
- 注册前采用最后一次有效邀请；过期、撤销或无效邀请不覆盖此前仍有效的候选邀请。拒绝手机号授权不锁定关系。
- 手机号首次注册成功后永久锁定邀请关系。同一新人最多触发一次邀请奖励；自邀不奖励邀请人，但不阻止正常登录或普通新人权益。
- 邀请人最多成功获得 5 次、累计 150 次；奖励是对原余额增加 30 次，不是重置余额。
- 推荐使用邀请分享凭证表和邀请注册关系表两张表，并采用两阶段幂等状态机：第一事务锁定手机号首次注册事实、普通新人权益、邀请关系和上限名额，第二事务原子写 `SHARE_REWARD` 流水并完成奖励状态；持久化待发状态由补偿任务重试。
- 邀请 token 与 `candidateReceipt` 均为 bearer credential，必须高熵生成、摘要存储并分别使用不与其他业务密钥复用的独立密钥。
- 第一批开发前仍需核对身份 store、权益 store 的连接池复用和事务连接注入方式。微信外部 HTTP 必须在数据库事务及行锁之外完成。

页面状态已更新：“获取学习权益”页中的会员购买和本机购买记录已经实现；邀请卡片、邀请进度、邀请落地页和活动规则页仍未实现。完成后页面顺序固定为会员购买、邀请新用户注册成功得30次、邀请进度、活动规则、本机购买记录。

### 2026-08-28 记录

完整产品规则以 `docs/CONTENT_ACCESS_POLICY.md` 的 F、J 节为准。本节仅记录实现状态和开发门槛。

以下是 2026-08-28 当时的规划快照，不代表 2026-09-23 的当前实现状态：当时统一“获取学习权益”页、邀请规则页和会员购买确认页均尚未实现。其后会员购买及本机购买记录已经实现，邀请卡片、邀请落地页和活动规则页仍未实现。

30 天会员属于数字虚拟权益。虚拟支付订单需要区分环境、渠道和平台，并关联平台订单、支付、发货、退款和会员 grant 发放结果。当前虚拟支付基础订单、会员发放、发货恢复、购买页面和本机购买记录已经实现；退款闭环、正式开放条件和生产验收仍未全部完成。
