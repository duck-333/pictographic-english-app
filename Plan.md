...
## 2026-07-16：用户系统 Phase 2 架构与开发顺序

目标：

- 在已完成微信登录、手机号快捷登录、`users`、`wechat_user_bindings`、`user_phone_bindings` 的基础上进入 Phase 2。
- Phase 2 拆分为五个模块：用户身份认证层、用户学习数据同步层、查词额度/权益系统、内容访问控制层、后台用户运营查询。
- MVP 阶段保证账号数据边界清晰：未登录学习数据只保存在本机；登录后学习数据从服务器侧 `users.id` 记录开始。
- 不实现游客数据自动迁移、确认导入、合并或关联。

开发顺序：

1. 用户身份认证层：补齐 `verifyUserSessionToken()` / `requireUserAuth()`，先验证 `GET /api/me`。
2. 用户学习数据同步层：新增服务器侧收藏、最近查看、每日学习统计能力，登录用户数据以 `users.id` 为准。
3. 查词额度/权益系统：先实现 `word_lookup` quota account 和 ledger，不提前实现完整会员/支付。
4. 内容访问控制层：新增受控完整详情入口，后续接入 quota 扣减。
5. 后台用户运营查询：在学习数据和 quota 数据形成后，先做只读查询，再评估运营写操作。
...
- 账号合并、解绑、换绑。
- 支付、订单、买书激活。
- 完整会员体系和 `user_entitlements` 实现。
- 后台人工调额写操作。

验收：

- 每个模块必须可独立验证、可回滚，不影响现有微信手机号登录链路。
- 任何数据库执行前必须遵循 ADR-0007，另行确认迁移脚本、回滚方案、备份和目标环境。
